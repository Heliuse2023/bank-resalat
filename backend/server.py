...
# Enhanced storage directories - Railway compatible
DATA_DIR = Path("./data")
...
try:
    ...
    if os.path.exists(static_path):
        ...
        logger.warning("Static directory not found - running API only")
        
except ImportError:
    logger.info("Running without static file serving")
except Exception as e:
    logger.error(f"Static file setup error: {e}")

# Enhanced CORS middleware with dynamic origin support
app.add_middleware(
    CORSMiddleware,
    allow_credentials=True,
    allow_origins=["*"],  # Allow all origins for flexible hosting
    allow_methods=["*"],
    allow_headers=["*"],
    expose_headers=["*"]
)
